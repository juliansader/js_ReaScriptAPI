...
APIdef aAPIdefs[] =
{
	{ APIFUND(JS_GetLevel), "int", "void*", "hwnd", "", },
	...
	{ APIFUNC(JS_Window_Create), "void*", "const char*,const char*,int,int,int,int,char*,void*", "title,className,x,y,w,h,styleOptional,ownerHWNDOptional", "Creates a modeless window with WS_OVERLAPPEDWINDOW style and only rudimentary features. Scripts can paint into the window using GDI or LICE/Composite functions.\n\nownerHWND: Optional parameter, usually either the REAPER main window or another script window, and useful for ensuring that the created window automatically closes when the owner is closed.\n\nstyle: An optional parameter that overrides the default style. The string may include any combination of standard window styles, such as \"OVERLAPPED,VSCROLL,HSCROLL\".\n\nWARNING: This function is not yet fully implemented on macOS and Linux. In particular, the ownerHWND and style parameters are ignored, and .", },
	...
	{ APIFUNC(JS_Window_SetZOrder), "bool", "void*,const char*,void*", "windowHWND,ZOrder,insertAfterHWND", "Sets the window Z order.\n\nParameters:\n * ZOrder: \"BOTTOM\", \"TOPMOST\", \"NOTOPMOST\", \"TOP\" or \"INSERT_AFTER\".\n * InsertAfterHWND: If ZOrder is INSERT_AFTER, insertAfterHWND must be a handle to the window to precede windowHWND in the Z order; otherwise, insertAfterHWND is ignored (in which case it can be set to the same value as windowHWND).", },
