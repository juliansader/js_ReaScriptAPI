...
APIdef aAPIdefs[] =
{
	...
	{ APIFUNC(JS_VKeys_GetState), "bool", "char*,int*", "stateOutNeedBig,stateOutNeedBig_sz", "Retrieves the current states (0 or 1) of all virtual keys, from 0x01 to 0xFF, in a 255-byte array.\n\nNotes:\n * Mouse buttons and modifier keys are not (currently) reliably detected, and JS_Mouse_GetState can be used instead.\n * Some platforms may continuously send VK messages to windows (for example, VK_SELECT on macOS) and a non-empty state therefore does not imply that the user pressed a key.", },
	{ APIFUNC(JS_VKeys_GetHistory), "bool", "char*,int*", "stateOutNeedBig,stateOutNeedBig_sz", "Retrieves the cumulative keystrokes (0 up to a maximum of 255 per key) of all virtual keys, from 0x01 to 0xFF, since the last JS_VKeys_ClearHistory, in a 255-byte array.\n\nNote: Mouse buttons and modifier keys are not (currently) reliably detected, and JS_Mouse_GetState can be used instead.", },
	{ APIFUNC(JS_VKeys_GetHistState), "bool", "char*,int*", "stateOutNeedBig,stateOutNeedBig_sz", "Returns a 255-byte array that specifies (0 or 1) which virtual keys, from 0x01 to 0xFF, have been pressed since the last JS_VKeys_ClearHistory.\n\nNote: Mouse buttons and modifier keys are not (currently) reliably detected, and JS_Mouse_GetState can be used instead.", },
