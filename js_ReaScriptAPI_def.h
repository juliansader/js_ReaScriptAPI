...
APIdef aAPIdefs[] =
{
	...
	{ APIFUNC(JS_Window_Create), "void*", "const char*,const char*,int,int,int,int,char*,void*", "title,className,x,y,w,h,styleOptional,ownerHWNDOptional", "Creates a modeless window with WS_OVERLAPPEDWINDOW style and only rudimentary features. Scripts can paint into the window using GDI or LICE/Composite functions.\n\nownerHWND: Optional parameter, only used on WindowsOS. Usually the REAPER main window, or another script window, and useful for ensuring that the created window automatically closes when the owner is closed.\n\nstyle: An optional parameter that overrides the default style, only used on WindowsOS. The string may include any combination of standard window styles, such as \"OVERLAPPED,VSCROLL,HSCROLL\".", },
